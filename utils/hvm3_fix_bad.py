...
def main():
    ...
    parser.add_argument('--saturation',type=int,default=14000)
    ...
    with open(args.input,'rb') as fin:
       ...
       spectra = frames.reshape((int(frames.size/rows),rows))
       use = np.all(spectra < args.saturation, axis=1)
       spectra = spectra[use,:]
    ...
    if len(args.output_bad) > 1:
        ...
        with open(args.output_bad+'.hdr','w') as fhout:
           fhout.write(header_string.format(**locals()))
