...
def dark_from_file(filepath, maskpath='', masked_columns=5):
    ...
    if len(maskpath)>0:
       maskfile = envi.open(find_header(maskpath))
       mask = np.squeeze(maskfile.load())
    else:
       mask = np.ones((lines,columns))
    if masked_columns>0:
       mask[:,:masked_columns] = 1
       mask[:,(-masked_columns):] = 1
    ...
    with open(filepath, 'rb') as fin:
        for line in range(lines):
            ...
            if line%10==0:
                ...
                print('Averaging line '+str(line))
            frame = sp.fromfile(fin, count=nframe, dtype=dtype)
            if len(frame)<nframe:
                print('exiting early')
                break
            frame = frame.reshape((rows, columns))
            masked = np.tile(mask[line,:],(rows,1))
            use = np.logical_and(frame>bad_flag,masked>0)
...
def main():
    ...
    parser.add_argument('--masked_columns',type=int,default=5)
    parser.add_argument('--mask',default='')
    parser.add_argument('output_dark')
    args = parser.parse_args()

    dark_avg, dark_std = dark_from_file(args.input, args.mask, 
        args.masked_columns)
